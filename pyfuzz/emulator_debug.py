...
class Emulator:
    ...
    def uc_code_cb(self, uc, addr, size, user_data):
        ...
        for insr in self.cs.disasm(code, addr, 2):
            check=False
            if insr.mnemonic=="bx" or ("pop" in insr.mnemonic and "pc" in insr.op_str):
                check=True
                print("0x%08x\t %s %s" %(addr, insr.mnemonic, insr.op_str))
                self.func_stack.pop(-1)
                print([hex(_) for _ in self.func_stack],len(self.func_stack))
            if insr.mnemonic in ["bl","bx","blx"] and insr.op_str!="lr":
                check=True
                print("0x%08x\t %s %s" %(addr, insr.mnemonic, insr.op_str))
                if insr.op_str[0]=="#":
                    self.func_stack.append(int(insr.op_str[1:],16))
                else:    
                    self.func_stack.append(uc.reg_read(globals()["UC_ARM_REG_"+insr.op_str.upper()]))
                print([hex(_) for _ in self.func_stack],len(self.func_stack))
                print(hex(addr)+": "+" ".join([hex(uc.reg_read(globals()[f"UC_ARM_REG_R{i}"])) for i in range(4)]))
            if insr.mnemonic == "b.w":
                check=True
                print("0x%08x\t %s %s" %(addr, insr.mnemonic, insr.op_str))
                self.func_stack.pop(-1)
                self.func_stack.append(int(insr.op_str[1:],16))
